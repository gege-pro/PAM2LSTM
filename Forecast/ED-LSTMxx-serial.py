...
# save model
model.save('../h5/ED-LSTMxx' + '-serial' + str(len_pre) + '.h5')
...
# model forecast
model.load_weights('../h5/ED-LSTMxx' + '-serial' + str(len_pre) + '.h5')
...
#
Pre_peak = signal.find_peaks(Prediction_Z, distance=100)
Real_peak = signal.find_peaks(Real_Z, distance=100)
PreTime, RealTime = [], []
pre_flood, real_flood = [], []
for i in Pre_peak[0]:
    if Prediction_Z[i] >= 34.5:
        PreTime.append(i)

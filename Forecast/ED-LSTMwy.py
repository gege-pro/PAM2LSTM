...
def create_dataset(data, look_back1, look_back2, look_back3, look_back4, pre_len):
    ...
    for i in range(len(data) - maxlen - pre_len - 1):
        ...
        temp = sequence.pad_sequences(temp, maxlen=max(look_back1, look_back2, look_back3), value=2000)
        X1.append(temp)
        X2.append(temp4)
    ...
    X1 = np.reshape(X1, newshape=(X1.shape[0], max(look_back1, look_back2, look_back3), 3))
    X2 = np.reshape(X2, newshape=(X2.shape[0], look_back4, 1))
    return X1, X2, Y
...
len_WK = 140
len_Z = 72
MaxLen = max(len_R, len_Q, len_WK)
len_pre = 6
Batch_size = 64
EPOCH = 100

# X_set, Z_set = create_dataset(data, len_R, len_Z, len_pre)
X1_set, X2_set, Z_set = create_dataset(data, len_R, len_Q, len_WK, len_Z, len_pre)
...
input_layer1 = tf.keras.layers.Input(shape=(MaxLen, 3))
# x1 = tf.keras.layers.Masking(mask_value=2000, input_shape=(MaxLen, 3))(input_layer1)
"""
x = tf.keras.layers.LSTM(units=180, activation='relu', input_shape=(MaxLen, 4))(input_layer)
x = tf.keras.layers.RepeatVector(n=len_pre)(x)
x = tf.keras.layers.LSTM(units=50, activation='tanh', return_sequences=True)(x)
output_layer = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(1))(x)
"""
x1 = tf.keras.layers.LSTM(units=64, activation='relu', input_shape=(MaxLen, 3))(input_layer1)
...
# save model
GRU_layer.save('../h5/ED-LSTMwy' + '-' + str(len_pre) + '.h5')
...
# forecast
GRU_layer.load_weights('../h5/ED-LSTMwy' + '-' + str(len_pre) + '.h5')
...
# preAndReal.to_csv('../h5/BiLSTM_P&S.csv')
